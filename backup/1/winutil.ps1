...
function Add-Button($name, $text, $x, $color, $w = 120) {
...
$btnCopyList = Add-Button 'copylist' 'Copy list'   224  $null
$btnSaveList = Add-Button 'savelist' 'Save list'   330  $null
$btnImport   = Add-Button 'import'   'Import list' 436  $null
...
$btnCopyList.Add_Click({
    ...
    $countLabel.Text = "$($script:allInstalled.Count) installed app(s) copied to clipboard"
})
...
$btnSaveList.Add_Click({
    ...
    if ($dlg.ShowDialog() -eq 'OK') {
        ...
        $countLabel.Text = "Saved $($script:allInstalled.Count) app(s) to $($dlg.FileName)"
    }
})
...
$btnImport.Add_Click({
    ...
    Update-Count
    $countLabel.Text = "Imported $($ids.Count) app(s) ($matched from this list, $($script:extraIds.Count) extra)"
})
